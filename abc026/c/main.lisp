...
(defun solve (n b)
  (let ((sub-list (make-array (1+ n)
                              :element-type 'list
                              :initial-element nil)))
    (declare ((array list 1) sub-list))
    (loop for i from 2 to n do
         (push i (aref sub-list (pop b))))
    (labels ((dfs (x &optional (pmax 0) (pmin (expt 10 9)))
               (if (null (aref sub-list x))
                   1
                   (loop for person in (aref sub-list x) do
                        (let ((p (dfs person)))
                          (setf pmax (max pmax p))
                          (setf pmin (min pmin p)))
                      finally
                        (return (1+ (+ pmax pmin)))))))
      (dfs 1))))
