...
(defun solve (sx sy gx gy)
  (let ((dx (- gx sx))
        (dy (- gy sy)))
    (concatenate 'string
                 (loop repeat dx collect #\R)
                 (loop repeat dy collect #\U)
                 (loop repeat dx collect #\L)
                 (loop repeat dy collect #\D)
                 '(#\D)
                 (loop repeat (+ dx 1) collect #\R)
                 (loop repeat (+ dy 1) collect #\U)
                 '(#\L)
                 '(#\U)
                 (loop repeat (+ dx 1) collect #\L)
                 (loop repeat (+ dy 1) collect #\D)
                 '(#\R))))
...
(defun main ()
  (declare #.OPT)
  (destructuring-bind (sx sy gx gy) (loop repeat 4 collect (read))
    (princ (solve sx sy gx gy))
    (fresh-line)))
