...
(defun solve (n k ratings)
  (declare (fixnum n k)
           (list ratings))
  (the double-float
       (labels ((inner (k ratings &optional (cnt 0) (res 0))
                  (declare (fixnum k cnt)
                           (rational res)
                           (list ratings))
                  (if (= cnt k)
                      (coerce res 'double-float)
                      (inner k (rest ratings) (1+ cnt) (/ (+ res
                                                             (first ratings))
                                                          2)))))
         (inner k (subseq ratings (- n k))))))
...
(defun main ()
  ...
  (let ((n (read))
        (k (read)))
    (declare (fixnum n k))
    (let ((ratings (fast-sort (read-numbers-to-list n))))
      (declare (list ratings))
      (println (solve n k ratings)))))
