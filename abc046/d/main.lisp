...
(defun solve (xs &optional (res 0) (cnt 0))
  (declare (list xs)
           (fixnum res cnt))
  (the fixnum
       (cond
         ((null xs) res)
         ((zerop cnt) (If (char-equal (first xs)
                                      #\p)
                          (solve (rest xs) (1- res) 1)
                          (solve (rest xs) res 1)))
         ((char-equal (first xs) #\p) (solve (rest xs) res 0))
         (t (solve (rest xs) (1+ res) 0)))))


(defun main ()
  (declare #.OPT)
  (let ((xs (concatenate 'list (read-line))))
    (declare (list xs))
    (princ (solve xs))
    (fresh-line)))
