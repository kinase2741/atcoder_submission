...
(defun solve (n k interval)
  (let ((dp (make-array (+ (* n 2) 10) :initial-element 0)))
    (setf (aref dp 0) 1)
    (setf (aref dp 1) -1)
    (loop for i below n
       do
         (when (plusp i)
           (incf (aref dp i) (aref dp (1- i))))
         (loop for m in interval do
              (incf (aref dp (+ i (first m)))
                    (aref dp i))
              (modf (aref dp (+ i (first m))) *mod*)
              (decf (aref dp (+ i (rest m) 1))
                    (aref dp i))
              (modf (aref dp (+ i (rest m) 1)) *mod*))
       finally
         (return (mod (+ (aref dp (1- n))
                         *mod*)
                      *mod*)))))
...
(defun main ()
  ...
  (let ((n (read))
        (k (read)))
    (let ((interval (loop repeat k collect (cons (read) (read)))))
      (princ (solve n k interval))
      (fresh-line))))
