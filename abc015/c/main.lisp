...
(defun solve (n k board)
  (let ((memo (make-hash-table :test #'equal)))
    (labels ((dfs (i x)
               (cond
                 ((zerop i) (zerop x))
                 ((cdr (gethash (list i x) memo))
                  (car (gethash (list i x) memo)))
                 (t
                  (setf (gethash (list i x) memo)
                        (some (lambda (q)
                                (dfs (1- i) (logxor x (aref board (1- i) q))))
                              (iota k)))))))
      (if (dfs n 0)
          "Found"
          "Nothing"))))
...
(defun main ()
  ...
  (let ((n (read))
        (k (read)))
    (let ((board (make-array (list n k))))
      (dotimes (i n)
        (dotimes (j k)
          (setf (aref board i j) (read-fixnum))))
      (princ (solve n k board))
      (fresh-line))))
