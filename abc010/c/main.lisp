...
(defun calc-squared-dist (x1 y1 x2 y2)
  (declare (fixnum x1 y1 x2 y2))
  (+ (expt (- x1 x2) 2)
     (expt (- y1 y2) 2)))

(defun judge (sx sy gx gy time speed point)
  (declare (fixnum sx sy gx gy time speed)
           (list point))
  (let ((maxlen (* speed time)))
    (<= (+ (sqrt (calc-squared-dist sx sy (first point) (rest point)))
           (sqrt (calc-squared-dist gx gy (first point) (rest point))))
        maxlen)))


(defun solve (sx sy gx gy time speed points)
  (declare (fixnum sx sy gx gy time speed)
           (list points))
  (if (some (lambda (point)
              (judge sx sy gx gy time speed point))
            points)
      "YES"
      "NO"))
...
(defun main ()
  (declare #.OPT)
  (destructuring-bind (sx sy gx gy time speed) (read-numbers-to-list 6)
    (let ((n (read)))
      (let ((points (loop repeat n collect (cons (read) (read)))))
    (format t "~a~&" (solve sx sy gx gy time speed points))))))
