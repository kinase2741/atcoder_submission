...
(defun solve (red-points blue-points)
  (loop
     for rp in red-points
     with res = 0
       with used = nil
     do
       (loop for bp in blue-points
          do
            (when (and
                   (not (find bp used :test #'equal))
                   (< (first rp) (first bp))
                   (< (rest rp) (rest bp)))
              (incf res)
              (push bp used)
...
(defun main ()
  (declare #.OPT)
  (let ((n (read)))
        (let ((red-points (sort (loop repeat n collect (cons (read) (read))) #'rest>))
            (blue-points (sort (loop repeat n collect (cons (read) (read))) #'first<)))
      (princ (solve red-points blue-points))
      (fresh-line))))
