...
(defun solve (n)
  (let ((r (rem n 30)))
    (let* ((s (truncate r 5))
           (card-list (loop for i below 6 collect (rem (+ i s) 6))))
      (dotimes (i (rem r 5))
        (rotatef (nth (mod i 5) card-list)
                 (nth (mod (1+ i) 5) card-list)))
      (setf card-list (mapcar #'1+ card-list))
      (format nil "~{~a~}" card-list))))
