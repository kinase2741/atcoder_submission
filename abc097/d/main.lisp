...
(defun main ()
  ...
  (let ((n (read))
        (m (read)))
    (declare  (fixnum n m))
    (let ((p (make-array n :initial-contents (loop repeat n collect (1- (read)))))
          (edges (loop repeat m collect (cons (1- (read)) (1- (read))))))
      (declare (array p)
               (list edges))
      (princ (solve n p edges))
      (fresh-line))))
