...
(defun calc-count (b d goal set &optional (res 0) (not-perf nil))
...
(defun solve (d goal set &optional (res 10000))
  (loop for i from 1 below (ash 1 d) do
       (setf res (min res (calc-count i d goal set)))
     finally
       (return res)))
...
(defun main ()
  ...
  (let* ((d (read))
         (goal (read))
         (set (loop repeat d collect (cons (read) (read)))))
    (format t "~a~&" (solve d goal set))))
