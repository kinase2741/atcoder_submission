...
(defun solve (n ng-list)
  (declare (fixnum n)
           (list ng-list))
  (the string
       (loop repeat 100 do
            (cond
              ((find n ng-list) (return "NO"))
              ((not (find (- n 3) ng-list)) (decf n 3))
              ((not (find (- n 2) ng-list)) (decf n 2))
              ((not (find (- n 1) ng-list)) (decf n 1))
              (t (return "NO")))
          finally
            (if (<= n 0)
                (return "YES")
...
(defun main ()
  (declare #.OPT)
  (let ((n (read))
        (ng-list (read-numbers-to-list 3)))
    (declare (fixnum n)
             (list ng-list))
    (format t "~a~&" (solve n ng-list))))
