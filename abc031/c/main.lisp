...
(defun calc-score (xs take-even &optional (res 0))
  (declare (list xs)
           (t take-even)
           (fixnum res))
  (if (null xs)
      res
      (if take-even
          (calc-score (rest xs) (null take-even) res)
          (calc-score (rest xs) (null take-even) (+ res (first xs))))))


(defun solve (n a)
  (declare (fixnum n)
           (list a))
  (loop for i from 0 below n with res = -5000 do
       (loop for j from 0 below n with tak-sum = -5000 with aoki-sum = -5000 do
            (unless (= i j)
              (let ((lst-t (subseq a (min i j) (1+ (max i j)))))
                (when (> (calc-score lst-t t) aoki-sum)
                  (setq aoki-sum (calc-score lst-t t))
                  (setq tak-sum  (calc-score lst-t nil)))))
              finally
              (setq res (max res tak-sum)))
          finally
            (return res)))
