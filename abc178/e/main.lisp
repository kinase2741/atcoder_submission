...
(defun solve (n points)
  (let ((x-dash (mapcar (lambda (p)
                          (+ (first p)
                             (rest p)))
                        points))
        (y-dash (mapcar (lambda (p)
                          (- (first p)
                             (rest p)))
                        points)))
    (let ((x-dash-max (reduce #'max x-dash))
          (x-dash-min (reduce #'min x-dash))
          (y-dash-max (reduce #'max y-dash))
          (y-dash-min (reduce #'min y-dash)))
      (max (abs (- x-dash-max x-dash-min))
           (abs (- y-dash-max y-dash-min))))))
